...
The current DistilBERT SST-2 sentiment analyzer was evaluated using a manually labeled sample dataset containing 30 product-review samples.

The evaluation dataset was balanced across the two supported sentiment classes:
...
Expected sentiment labels were compared with predictions generated by the same `analyse_sentiment()` function used by SmartReview.
...
The reported accuracy applies only to this manually labeled binary evaluation dataset.

It should not be interpreted as universal model accuracy.

A larger independent evaluation dataset containing more ambiguous, sarcastic, and mixed-sentiment reviews would provide a more comprehensive assessment.
...
- The current sentiment model performs binary sentiment classification and does not produce a neutral sentiment label.
...
The final local demo dataset was recreated after updating the sentiment model and generated only through the current DistilBERT SST-2 sentiment analysis pipeline.
...
- Transformer-based binary sentiment analysis
...
The project originally used the Cardiff NLP RoBERTa sentiment model, which supports positive, negative, and neutral sentiment classification. During deployment testing on free-tier cloud platforms, the model's memory requirements exceeded the available resources.

To improve deployment compatibility, the application was updated to use the DistilBERT SST-2 sentiment model. This significantly reduces memory usage while maintaining Transformer-based sentiment analysis, but changes sentiment classification from three classes (positive, negative, neutral) to two classes (positive, negative).